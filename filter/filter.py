...
class FilterEnglish(Filter):
    ...
    def __init__(self, path_to_stop_words_trie=None):
        ...
        if not path_to_stop_words_trie:
            self.stop_words_trie = trie.marisa_trie.Trie(stopwords.words("english"))
    ...
    def get_all_tokens(self, text):
        return [
            self.lexer__.lemmatize(
                word, pos=self.lexer__.get_wordnet_pos(speech_part))
...
class FilterRussian(Filter):
    ...
    def __init__(self, path_to_stop_words_trie=None):
        ...
        if not path_to_stop_words_trie:
            self.trie = trie.marisa_trie.Trie(stopwords.words("russian"))
    ...
    def get_tokens(self, text):
        return [lex for lex in
                [self.get_word_from_token(word)
                 for word in self.stemmer_.analyze(text)] if lex is not None]
